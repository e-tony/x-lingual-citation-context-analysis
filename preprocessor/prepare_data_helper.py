...
def load_dataset(path):
    ...
    names = None if '.tsv' in path else ['uuid','lang','ctd_mid','ctg_mid','ctd_aid','ctg_aid','published','context']
...
def make_sample(*, xling_path, adj_path):
    ...
    xling_names = ['uuid', 'ctd_mag_id', 'adj_mid_str', 'ctg_mid', 'ctd_aid', 'adj_aid_str', 'ctg_aid', 'context']
    adj_names = ['lang', 'uuid', 'citing_mid', 'cited_mid', 'citing_aid', 'cited_aid', 'bibitem_str']
    ...
    for name, group in adj_contexts_df.groupby(['citing_aid']):
        assert (group.dropna(subset=['lang']).shape[0] > 0)  # make sure at least one item has a language
        ...
        for j,lang_id in enumerate(lang_ids):
            if non_lang_ids:
                ...
                print(f'Document with citing id: {citing_aid} has a debt of {str(len(lang_ids)-num_samples)} samples.')
...
def filter_db(*, db_path, citations_path):
    ...
    # search statement
    stmt = text('select uuid, citing_mag_id, cited_mag_id, citing_arxiv_id, cited_arxiv_id, bibitem_string from bibitem;')
    ...
    with db_engine.connect() as conn:
        ...
        for row in rows:
            uuid, citing_mad_id, cited_mag_id, citing_arxiv_id, cited_arxiv_id, bibitem = row
...
def format_file(*, path, xling_path=None):
    ...
    for i,row in df.iterrows():
        ...
        if xling_path:
            ...
            new_row = [uuid, lang, ctd_mid, ctg_mid, ctd_aid, ctg_aid, published, category, context]
            rows.append(new_row)
        else:
