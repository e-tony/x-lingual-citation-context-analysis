...
class CitationClassificationModel:
    def __init__(self, model_name, config, output_dir=None, debug=False):
        ...
        self.do_lower_case = (
            True if self.config["PARAM"]["do_lower_case"] == "True" else False
        )
    ...
    def train(self, train_dataset, eval_dataset, hp_search):
        ...
        if hp_search:
            ...
            logging.info(
                f'Eval loss: {res["eval_loss"]}, Eval acc: {res["eval_accuracy"]}, Eval F1: {res["eval_f1"]}, Eval P: {res["eval_precision"]}, Eval R: {res["eval_recall"]}'
            )
    ...
    def predict(self, dataset, model_path=None, get_performance=True):
        ...
        classifier = TextClassificationPipeline(
            model=self.model, tokenizer=self.tokenizer, return_all_scores=True, device=0
        )
        ...
        if target_names:
            report = classification_report(
                true.tolist(),
                torch.argmax(pred, axis=1).tolist(),
