...
class CitationClassifier:
        ...
        if self.dataset_name == 'Athar':
            ...
            if path.is_file():
                if not train_path.is_file() or not val_path.is_file() or not test_path.is_file():
                    ...
                    df_train.to_csv(str(train_path), sep='\t', index=False, header=False)
                ...
                dataset_dict = datasets.DatasetDict({'train': train_dataset, 'validation': val_dataset, 'test': test_dataset})

                self.dataset = Dataset(self.config)
                self.dataset.dataset = dataset_dict
                self.dataset.format_data(self.classifier.tokenizer)
            else:
                raise Exception(f'Cannot find dataset {self.dataset_name} with path: {str(path)}')
        else:
            self.dataset = Dataset(self.config)
    ...
    def load_model(self, model_path=None):
        ...
        self.classifier = CitationClassificationModel(model_name, self.config, output_dir=self.output_dir, debug=self.args.debug)
